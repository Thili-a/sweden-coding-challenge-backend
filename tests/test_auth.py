...
def test_create_user_success(client):
    ...
    response = client.post("/user", json=new_user_data, headers={"access-token": pytest.access_token})
...
def test_delete_user_fail1(client, app):
    with app.app_context():
        response = client.delete("/user/test_id", headers={"access-token": pytest.access_token})
        assert b"User Not found!" in response.data
        assert response.status_code == 404
...
def test_delete_user_success(client, app):
    with app.app_context():
        user = User.query.filter_by(email="aaa@gmail.com").first()
        public_id = user.public_id
        response = client.delete("/user/"+public_id, headers={"access-token": pytest.access_token})
